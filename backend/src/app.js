...
app.post("/extract-content", requireAuth, upload.single("pdf"), asyncHandler(extractContent));
app.get("/extract-content-status", requireAuth, asyncHandler(extractionStatus));
app.post("/generate-quiz", requireAuth, rateLimit({ windowMs: 60_000, max: 8, label: "generate-quiz" }), asyncHandler(generateQuiz));
app.post("/submit-quiz", requireAuth, asyncHandler(submitQuiz));
app.post("/generate-flashcards", requireAuth, rateLimit({ windowMs: 60_000, max: 8, label: "generate-flashcards" }), asyncHandler(generateFlashcardsController));
