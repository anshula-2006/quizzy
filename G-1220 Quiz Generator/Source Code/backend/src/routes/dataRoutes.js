...
import { bootstrapUserData, clearAttempts, clearDashboard, createFlashDeck, createSavedQuestion, generateTeacherExplanation, listGlobalQuizzes, publishGlobalQuiz, reportQuestion, startGlobalQuiz, teacherDashboardData, updateMiniGameStats, updateFlashDeck, deleteFlashDeck } from "../controllers/quizController.js";
...
router.post("/global-quizzes", requireAuth, rateLimit({ windowMs: 60_000, max: 12, label: "publish-global-quiz" }), asyncHandler(publishGlobalQuiz));
router.post("/global-quizzes/explain", requireAuth, rateLimit({ windowMs: 60_000, max: 10, label: "teacher-explain" }), asyncHandler(generateTeacherExplanation));
router.post("/global-quizzes/:id/start", requireAuth, asyncHandler(startGlobalQuiz));
router.post("/question-reports", requireAuth, rateLimit({ windowMs: 60_000, max: 10, label: "question-report" }), asyncHandler(reportQuestion));
